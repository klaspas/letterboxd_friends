...
const getUser = () => {
    const match = document.cookie.split('; ').find((row) => row.startsWith('letterboxd.signed.in.as='));
...
const parseLikes = (ratingsHtml) => {
    ...
    return likesMatch ? Number(likesMatch[1]) : 0;
};
...
const fetchRatingsData = async (user, movie) => {
    ...
    const [ratingsHtml, histogramHtml] = await Promise.all([getHTML(ratingsUrl), getHTML(histogramUrl)]);
...
const getRelativeAndPercentRatings = (ratingCounts, votes) => {
    ...
    const relative = ratingCounts.map((count) => {
        const height = (count / maxRating) * 44.0;
        return height < 1 || !isFinite(height) ? 1 : height;
    });
    const percent = ratingCounts.map((count) => (votes > 0 ? Math.round((count / votes) * 100) : 0));
...
const formatRating = (i) => {
    ...
    return `${Math.floor(rating)}${encodeURIComponent('½')}`;
};

const buildHTMLStructure = (hrefHead, hrefLikes, likesText, avg1, dataPopup, ratingCounts, ratingLabels, maxRating) => {
    const starPath =
        'M5.065.45c-.22-.61-.95-.59-1.14 0l-.75 2.57H.705c-.73 0-.96.62-.37 1.07l1.99 1.53-.76 2.49c-.22.73.34 1.16.93.71l2-1.53 2 1.53c.59.45 1.15.02.93-.71l-.76-2.49 1.99-1.53c.59-.45.39-1.07-.33-1.07h-2.48z';
        ...
        <section class="section ratings-histogram-chart">
            ...
            <div class="rating-histogram"><div class="layout">
                ...
                <table class="chart"><caption class="_sr-only">Rating Distribution</caption>
                    ...
                    <tbody class="plot">${barsHtml}</tbody>
                </table>
                ${svgEnd}
